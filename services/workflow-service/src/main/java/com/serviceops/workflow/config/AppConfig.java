package com.serviceops.workflow.config;
...
public class AppConfig {
  ...
  SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
    http.csrf(csrf -> csrf.disable())
      .authorizeHttpRequests(auth -> auth.requestMatchers("/actuator/health").permitAll().requestMatchers(HttpMethod.GET, "/api/workflow/**").authenticated().requestMatchers(HttpMethod.POST, "/api/workflow/**").authenticated().anyRequest().authenticated())
