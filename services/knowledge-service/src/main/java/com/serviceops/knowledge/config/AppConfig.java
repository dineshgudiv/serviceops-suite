package com.serviceops.knowledge.config;
...
public class AppConfig {
  ...
  SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
    http.csrf(csrf -> csrf.disable())
      .authorizeHttpRequests(auth -> auth.requestMatchers("/actuator/health").permitAll().requestMatchers(HttpMethod.GET, "/api/knowledge/**").authenticated().requestMatchers(HttpMethod.POST, "/api/knowledge/**").authenticated().anyRequest().authenticated())
