package com.serviceops.sla.config;
...
public class AppConfig {
  ...
  SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
    http.csrf(csrf -> csrf.disable())
      .authorizeHttpRequests(auth -> auth.requestMatchers("/actuator/health").permitAll().requestMatchers(HttpMethod.GET, "/api/sla/**").authenticated().requestMatchers(HttpMethod.POST, "/api/sla/**").authenticated().anyRequest().authenticated())
