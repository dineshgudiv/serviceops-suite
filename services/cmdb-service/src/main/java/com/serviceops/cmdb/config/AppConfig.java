package com.serviceops.cmdb.config;
...
public class AppConfig {
  ...
  SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
    http.csrf(csrf -> csrf.disable())
      .authorizeHttpRequests(auth -> auth.requestMatchers("/actuator/health").permitAll().requestMatchers(HttpMethod.GET, "/api/cmdb/**").authenticated().requestMatchers(HttpMethod.POST, "/api/cmdb/**").authenticated().anyRequest().authenticated())
