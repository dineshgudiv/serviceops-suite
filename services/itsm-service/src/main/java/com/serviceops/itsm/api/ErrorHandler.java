...
public class ErrorHandler {
  ...
  public ResponseEntity<Map<String, Object>> generic(Exception ex, jakarta.servlet.http.HttpServletRequest req) {
    return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(Map.of("error", Map.of("code", "ITSM_INTERNAL", "message", ex.getMessage()), "request_id", String.valueOf(req.getAttribute("request_id"))));
  }
}
